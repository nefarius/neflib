...
namespace nefarius::devcon
{
	...
	std::expected<void, utilities::Win32Error> AddDeviceClassFilter(
        const GUID* classGuid, const std::wstring& filterName, DeviceClassFilterPosition position);

    std::expected<void, utilities::Win32Error> RemoveDeviceClassFilter(
        const GUID* classGuid, const std::wstring& filterName, DeviceClassFilterPosition position);

    std::expected<bool, utilities::Win32Error> HasDeviceClassFilter(const GUID* classGuid, const std::wstring& filterName,
